...
- **Peak-rate nudge** — if the washing machine, dishwasher or tumble dryer starts a cycle during the 17:00–19:00 peak electricity window, the speakers announce it and both phones get a push suggesting the run could wait until after 19:00
...
- **House Mode** — Tracks the occupancy and activity state of the household via `input_select.house_mode` (Home / Away / Sleeping) with overlays for storm (auto from Met Éireann) and maintenance (blocks Away, silences infra alerting during planned work, auto-expires 4 h). **Away** is set when both Colm and Olivia are out and no indoor motion remains — it won't trip if someone is still in the house even if phones suggest otherwise; Cian is excluded as he has no phone. **Sleeping** is detected automatically between 22:00 and 07:00 once the TVs and sitting-room lights have been off and no motion is seen in the sitting room, office, hall or landing for 20 minutes, with at least one adult home. **Home** resumes on the first TV ping, landing motion (morning) or sunrise. House mode is the single source of truth consumed by other flows to adapt their behaviour.
- **House Alarm** — Driven by `input_select.house_mode`: **Away** arms the house alarm in away mode, **Sleeping** arms in night mode, **Home** disarms it. When `input_boolean.guest_mode` is on, only **Away** arming is suspended (guests moving around would trip an away-armed alarm) — **night arming still happens** so the perimeter stays armed overnight with guests in; turning guest mode off immediately re-arms to match the current house mode. A welcome-home announcement plays when returning from Away (not from Sleeping). Every alarm event — armed, disarmed, triggered, cleared and failed-to-arm — across both the house and shed zones is pushed to Colm's and Olivia's phones and announced on all speakers, however the alarm was changed (phone, NFC or automatically).
- **Heating Control** — Driven by `input_select.house_mode` + time of day, setting the local HomeKit thermostat (the Netatmo holds a flat baseline; Node-RED only ever raises above it). **Home** → comfortable through the day, warmest between 7pm and 10pm. **Sleeping** → a lower overnight temperature, warming up from 7am so the house is ready when everyone wakes. **Away** → eases back to the baseline, dropping further after a full day empty (a toggle lets this deep setback be switched off for long absences with pets or plants). **Proximity pre-heat** — when the house is empty and someone is driving home (within ~12 km and getting closer, via the Proximity integration), it starts warming toward comfort so it's ready on arrival. **Forecast pre-heat** — each night it checks the Met Éireann forecast and, on a cold morning, starts the warm-up earlier (the colder it is, the earlier) so the house actually reaches temperature by wake time; when the forecast is below freezing it also pushes a heads-up the night before. **Boost** — nudging the thermostat holds your temperature for two hours before the schedule resumes (and cancels if everyone leaves). **Guest mode** — while guest mode is on the heating never drops to the Away setback, so visitors are never left cold; the normal overnight and morning behaviour still applies. The target is re-asserted every 30 minutes so the setpoint never lapses, and it recovers cleanly after a reboot.
