...
- **Heating Control** — Driven by `input_select.house_mode` + time of day, setting the local HomeKit thermostat (the Netatmo holds a flat baseline; Node-RED only ever raises above it). **Home** → comfortable through the day, warmest between 7pm and 10pm. **Sleeping** → a lower overnight temperature, warming up from 7am so the house is ready when everyone wakes. **Away** → eases back to the baseline, dropping further after a full day empty (a toggle lets this deep setback be switched off for long absences with pets or plants). **Forecast pre-heat** — each night it checks the Met Éireann forecast and, on a cold morning, starts the warm-up earlier (the colder it is, the earlier) so the house actually reaches temperature by wake time, with a heads-up pushed the night before. **Boost** — nudging the thermostat holds your temperature for two hours before the schedule resumes (and cancels if everyone leaves). The target is re-asserted every 30 minutes so the setpoint never lapses, and it recovers cleanly after a reboot.
